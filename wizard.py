...
class Wizard:
    ...
    def __str__(self):
        return "Wizard[{0}]: '{1}' ({2})".format(
            self.id, self.name, 'active' if self.is_active else 'inactive')
    ...
    def set_configuration(self, content):
        if self.configuration != content:
            r = self.broker.put('/wizards/{0}/configuration'.
    ...
    def validate_configuration(self, content):
        """
        Validates a wizard configuration and returns possible errors in a list
        of strings.
...
class FileSystem:
    ...
    def update_wizard_folders(self, wizards):
        for wizard in wizards:
            wizard_dir = '{0}{1} - {2}'\
                .format(self.path, wizard.id, wizard.name)
            # If the folder does not exist, create it
            if not os.path.isdir(wizard_dir):
                print("New wizard found: '{0}'".format(wizard.name))
                os.makedirs(wizard_dir)
            files = [(wizard_dir + '/WizardConfiguration.xml',
                      wizard.get_configuration()),
                     (wizard_dir + '/EventTemplate.xml',
                      wizard.get_event_template())]
            for (filePath, content) in files:
                # If the files do not exist, create them
                if not os.path.exists(filePath):
                    print("Downloading file '{0}' for wizard '{1}'"
                          .format(filePath, wizard.name))
                    ...
                    if not self.compare_file_with_data(filePath, content):
                        ...
                        while q != 'y' and q != 'n':
                            q = input(
                                'The file {0} has been modified locally. Overrwite? (y/n)'
